...
trait Monad[M[_]] extends Functor[M] {
  ...
  // also we can write 
  // la.map(f).sequence
  // but this would traverse the sequence twice
  ...
  def filterM[A](ms: List[A])(f: A => M[Boolean]): M[List[A]] = 
    ms.foldRight(unit(List[A]()))((x, y) => map2(f(x), y)((a, b) => if (a) (x :: b) else b))

  def compose[A,B,C](f: A => M[B], g: B => M[C]): A => M[C] = a => flatMap(f(a))(g)

  // Implement in terms of `compose`:
  // () is the instance of unit?
...
object Monad {
  ...
  val parMonad: Monad[Par] = new Monad[Par] {
    ...
    def flatMap[A, B](ma: Par.Par[A])(f: A => Par.Par[B]): Par.Par[B] = ma.flatMap(f)
  }
